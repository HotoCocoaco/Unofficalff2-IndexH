...
public void OnPluginStart()
{
	...
	//cvarEnableEurekaEffect = CreateConVar("ff2_enable_eureka", "0", "0-Disable the Eureka Effect, 1-Enable the Eureka Effect", _, true, 0.0, true, 1.0);	//传送扳手的Convar
	...
	RegConsoleCmd("comp", CompanionMenu, "Toggle being a FF2 companion");
...
public void EnableFF2()
{
	...
	SetConVarFloat(FindConVar("tf_feign_death_activate_damage_scale"), 0.14); //假死之铃抗性
...
public Action TF2Items_OnGiveNamedItem(int client, char[] classname, int iItemDefinitionIndex, Handle &item)
{
	...
	if(GetConVarInt(cvarHardcodeWep)>0)
	{
		switch(iItemDefinitionIndex)
		{
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 58 ; 1.5 ; 207 ; 0.9", true);
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "1 ; 0.5 ; 107 ; 1.65 ; 128 ; 1 ; 191 ; -20 ; 772 ; 8 ; 177 ; 4.5", true);
				//1: -50% damage
				//107: +100% move speed
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "140 ; 30.0");
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 730:  //乞丐的火箭筒
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 1 ; 76 ; 3 ; 2 ; 2.3 ; 16 ; 30 ; 394 ; 0.3 ; 413 ; 1 ; 417 ; 1 ; 241 ; 1.3 ; 424 ; 0.75 ; 411 ; 3 ; 421 ; 1", true);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "16 ; 40 ; 182 ; 3 ; 191 ; -16 ; 177 ; 1.35 ; 737 ; 3 ; 204 ; 1", false);
			...
			/*case 37, 1003:  //Ubersaw 过载锯子 2
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "522 ; 1 ; 255 ; 3", false);
				//522攻击造成airblast 255：airblast动力*3
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 44:  //Sandman 睡魔
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "6 ; 0.8 ; 360 ; 1.0 ; 38 ; 1 ; 279 ; 3 ; 2 ; 1.1", false);
				//6: +15% firing speed
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "26 ; 70 ; 53 ; 1");  //+50 health
			//53: 不会被点燃
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
			...
			case 220:  //游击手
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "125 ; -124 ; 525 ; 1.5 ; 534 ; 1.5 ; 800 ; 5 ; 275 ; 1 ; 138 ; 1.5 ; 70 ; 3 ; 239 ; 1.3", true);  
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.5 ; 17 ; 0.15 ; 318 ; 0.7 ; 26 ; 60 ; 177 ; 1.1");
				//2: +20% damage
				//17: +15% uber on hit
				//318: 装弹速度+30%
				if(itemOverride != INVALID_HANDLE)
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "177 ; 1.1 ; 5 ; 1.3");
			...
			case 142:  //机械臂 枪炮工之臂
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "26 ; 90 ; 5 ; 0.7 ; 16 ; 60 ; 124 ; 1 ; 321 ; 0.7 ; 287 ; 1.3 ; 107 ; 1.2 ; 737 ; 3");
				//26: +90 max health
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "113 ; 50");
			...
			case 527:  //寡妇
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "298 ; 20 ; 614 ; 1 ; 732 ; 0.7 ; 178 ; 0.5 ; 45 ; 1.3 ; 2 ; 1.8");
				//298: 每一发-10金属
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "205 ; 1 ; 206 ; 2.0 ; 772 ; 2.0", true);
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 2.4 ; 3 ; 0.5 ; 114 ; 1 ; 179 ; 1 ; 547 ; 0.6", true);
				//2: +10% damage bonus
				//3: -50% clip size
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "279 ; 12 ; 16 ; 10");
				//279: 5 ornaments
				//16: 击中后+10血
				if(itemOverride != INVALID_HANDLE)
				{
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "62 ; 0.7 ; 52 ; 1");
			...
			case 608, 405:  //呆毛鞋子
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "26 ; 100 ; 202 ; 3", false);
				//26: +100 max HP
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "204 ; 1 ; 44 ; 1 ;199 ; 0 ; 547 ; 0 ; 358 ; 0 ; 362 ; 0 ; 363 ; 0 ; 369 ; 0 ; 58 ; 1.55 ; 207 ; 2.3", true);
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 2 ; 125 ; -100 ; 394 ; 0.85 ; 418 ; 1 ; 532 ; 0.5 ; 275 ; 1 ; 76 ; 1.49 ; 78 ; 1.5 ; 737 ; 1");
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "179 ; 1 ; 2 ; 1.9 ; 76 ; 1.49 ; 97 ; 0.8");
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "92 ; 3.3 ; 396 ; 0.28 ; 95 ; 0.5 ; 1 ; 0.75 ; 59 ; 0.4 ; 287 ; 1.1 ; 287 ; 1.2", true);
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 437 ; 1 ; 182 ; 3 ; 737 ; 2 ; 218 ; 2", false);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 1178:  //龙之怒焰
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.6 ；109 ；1.2 ; 105 ; 1.5 ; 255 ; 1.33 ; 299 ; 1 ; 209 ; 1 ; 114 ; 1 ; 2 ; 2.8", false);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 425:  //家族运营枪
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "25 ; 1.6 ; 138 ; 1.1 ; 138 ; 1.4", false);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 155:  //南部的款待
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "179 ; 1 ; 204 ; 1 ; 339 ; 1 ; 340 ; 1 ; 345 ; 1.33 ; 58 ; 1.4 ; 149 ; 1 ; 207 ; 0.6 ; 344 ; 1.3 ; 287 ; 1.1 ; 287 ; 1.2", true);
				//179: Crit instead of mini-critting
				//204: 我打我自己
				//339&340: 让spy现身
				//345: 补给器+33%范围
			...
			case 996:  //脱缰巨炮
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 76 ; 3 ; 16 ; 70 ; 619 ; 1 ; 2 ; 2.3", false);
					//参考下面双管榴弹
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
			...
			case 448:  //苏打炮手
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 2 ; 76 ; 1.2 ; 78 ; 1.5");
				//2: 伤害+10%
				//97: 几乎瞬间换弹
				//76: 子弹+20%
				if(itemOverride != INVALID_HANDLE)
			...
			case 141, 1004:  //边疆正义 边境裁决者
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.8");
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 1078:  //节日自然之力
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "44 ; 1 ; 6 ; 0.5 ; 45 ; 1.2 ; 3 ; 0.34 ; 43 ; 1 ; 1 ; 0.9 ; 2 ; 2.5 ; 547 ; 0.9 ; 76 ; 1.2 ; 78 ; 1.5", true);
				//参考下面普通的自然之力
			...
			case 133:  //Gunboat 炮艇
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "26 ; 30", false);
			...
			case 1179:  //热能推进器
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "26 ; 30 ; 275 ; 1", false);
			...
			case 327:  //摩尔巨剑
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "202 ; 3 ; 778 ; 0.5 ; 781 ; 72 ; 412 ; 1.15 ; 551 ; 1", true);
			...
			case 30665:  //流星狙击枪
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5 ; 304 ; 4.3 ; 91 ; 0.15 ; 2 ; 1.2", false);
			...
			case 304:  //器官切割器
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "110 ; 50 ; 8 ; 1.4 ; 201 ; 1.7", false);
			...
			case 173:  //维他锯
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "366 ; 3 ; 877 ; 1 ; 19 ; 1.5", false);
			...
			case 237:  //火箭伞兵
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 1 ; 76 ; 3 ; 2 ; 2.3 ; 16 ; 30", true);
			...
			case 265:  //粘弹跳跃者
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 16 ; 30 ; 78 ; 3 ; 135 ; 0.6 ; 2 ; 2.54 ; 280 ; 14", true);
			...
			case 1098:  //经典狙击步枪
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5 ; 2 ; 1.05 ; 305 ; 1 ; 5 ; 1.2", false);
			...
			case 153, 466:  //Homewrecker 拆迁者
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "394 ; 3 ; 215 ; 30 ; 522 ; 1 ; 216 ; 30");
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 406:  //辉煌护盾
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "248 ; 2.6");
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 230:  //The Sydney Sleeper 悉尼沉睡者
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "42 ; 1 ; 175 ; 5 ; 15 ; 0 ; 41 ; 1.25 ; 2 ; 1.2 ; 76 ; 1.3 ; 16 ; 5", true);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 41:  //Natascha 娜塔莎转轮机枪
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "76 ; 5 ; 32 ; 0.6 ; 16 ; 3 ; 6 ; 0.7 ; 1 ; 0.7 ; 26 ; 50 ; 75 ; 1.6", true);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 312:  //Brass Beast 黄铜猛兽
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 2.0 ; 86 ; 3.3 ; 183 ; 0.1 ; 738 ; 0.4 ; 112 ; 0.1", true);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			case 1146:  //节日偷袭火焰
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.03 ; 255 ; 6 ; 209 ; 1 ; 76 ; 1.5 ; 165 ; 1.0 ; 171 ; 6 ; 2 ; 1.7", false);
				if(itemOverride != INVALID_HANDLE)
				{
					item=itemOverride;
					return Plugin_Changed;
			...
			{
				Handle itemOverride=PrepareItemHandle(item, _, _, "2 ; 2.4");
		...
		if(TF2_GetPlayerClass(client)==TFClass_Spy && (!StrContains(classname, "tf_weapon_invis", false))) //隐形手表
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "773 ; 1.3 ; 159 ; 1 ; 34 ; 2.1", false);
				//773：部署速度慢30%
				
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "728 ; 1 ; 35 ; 1.2 ; 160 ; 1 ; 253 ; -0.3", false); //隐身手表
				//728:隐身时不能吃弹药包
			}
		...
		if(TF2_GetPlayerClass(client)==TFClass_Spy && (!StrContains(classname, "tf_weapon_revolver", false))) //间谍手枪
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==24)  
			{
				itemOverride=PrepareItemHandle(item, _, _, "138 ; 1.4", false);
		...
		if(TF2_GetPlayerClass(client)==TFClass_Sniper && (!StrContains(classname, "tf_weapon_compound_bow", false))) //弓箭
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==56)  
			{
				itemOverride=PrepareItemHandle(item, _, _, "2 ; 3 ; 76 ; 2 ; 110 ; 25 ; 149 ; 3", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "2 ; 3 ; 76 ; 2 ; 110 ; 25 ; 149 ; 3", false);
		...
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==40)  
			{
				itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.03 ; 255 ; 6 ; 209 ; 1 ; 76 ; 1.5 ; 165 ; 1.0 ; 171 ; 6 ; 2 ; 1.7", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "220 ; 0.03 ；109 ；1.2 ; 105 ; 1.5 ; 255 ; 1.33 ; 76 ; 1.5 ; 171 ; 3 ; 2 ; 1.9", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_Medic && !StrContains(classname, "tf_weapon_syringegun_medic"))  //Syringe guns 注射枪
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "17 ; 0.05 ; 144 ; 1 ; 16 ; 5 ; 26 ; 60 ; 2 ; 1.5");
				//17: 5% uber on hit
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		else if(TF2_GetPlayerClass(client)==TFClass_Medic && !StrContains(classname, "tf_weapon_medigun"))  //Medi Gun 医疗枪
		{
			Handle itemOverride=PrepareItemHandle(item, _, _, "144 ; 2.0 ; 199 ; 0.75 ; 314 ; 4 ; 123 ; 1.2 ; 8 ; 1.6 ; 499 ; 50.0", true);
			if(itemOverride != INVALID_HANDLE)
			{
				item = itemOverride;
				return Plugin_Changed;
		...
		if(TF2_GetPlayerClass(client)==TFClass_Scout && (!StrContains(classname, "tf_weapon_scattergun", false))) //猎枪
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "2 ; 2.5 ; 547 ; 0.9 ; 76 ; 1.2 ; 78 ; 1.5", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "97 ; 0.8 ; 2 ; 2.7 ; 76 ; 1.36 ; 78 ; 1.5", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==7)  
			{
				itemOverride=PrepareItemHandle(item, _, _, "287 ; 2.2 ; 276 ; 1", true);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "287 ; 2.2 ; 276 ; 1", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_pipebomblauncher", false))) //粘弹发射器
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 16 ; 30 ; 78 ; 3 ; 135 ; 0.6 ; 2 ; 2.16", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 16 ; 30 ; 78 ; 3 ; 135 ; 0.6 ; 2 ; 2.54", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_grenadelauncher", false))) //榴弹发射器
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 76 ; 3 ; 16 ; 70 ; 619 ; 1 ; 179 ; 1 ; 2 ; 2.3", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "57 ; 1 ; 76 ; 3 ; 16 ; 70 ; 619 ; 1 ; 2 ; 1.9 ; 99 ; 0.9", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_DemoMan && (!StrContains(classname, "tf_weapon_sword", false))) //长剑
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==404)  //波斯弯刀
			{
				itemOverride=PrepareItemHandle(item, _, _, "772 ; 1.15 ; 249 ; 0.6 ; 781 ; 1 ; 778 ; 0.5 ; 782 ; 1 ; 16 ; 80", true);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "220 ; 150 ; 16 ; 80 ; 737 ; 3", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_Engineer && (!StrContains(classname, "tf_weapon_pistol", false))) //工程师手枪
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==22)  
			{
				itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.3", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "2 ; 1.3", false);
		...
		if(TF2_GetPlayerClass(client)==TFClass_Sniper && (!StrContains(classname, "tf_weapon_sniperrifle", false))) //狙击步枪
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5 ; 304 ; 4.3 ; 91 ; 0.15 ; 2 ; 1.2", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "16 ; 10 ; 76 ; 1.5 ; 2 ; 1.5", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_Heavy && (!StrContains(classname, "tf_weapon_minigun", false))) //转轮机枪
		{
			...
			{
				itemOverride=PrepareItemHandle(item, _, _, "16 ; 5 ; 20 ; 1 ; 77 ; 0.5 ; 431 ; 35 ; 112 ; 0.3 ; 26 ; 50 ; 208 ; 1 ; 2 ; 1.7", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "112 ; 0.1 ; 149 ; 3 ; 16 ; 3 ; 26 ; 50 ; 138 ; 1.6 ; 375 ; 50", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
		...
		if(TF2_GetPlayerClass(client)==TFClass_Soldier && (!StrContains(classname, "tf_weapon_rocketlauncher", false))) //士兵 火箭发射器
		{
			Handle itemOverride;
			if(iItemDefinitionIndex==127)  //Direct Hit 精确打击者
			{
				itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 179 ; 1.0 ; 57 ; 1 ; 76 ; 3 ; 2 ; 2.6 ; 16 ; 50 ; 245 ; 1.6 ; 244 ; 0.05", false);
			}
			else
			{
				itemOverride=PrepareItemHandle(item, _, _, "114 ; 1 ; 57 ; 1 ; 76 ; 3 ; 2 ; 2.3 ; 16 ; 30", false);
			}
			if(itemOverride != INVALID_HANDLE)
			{
				item=itemOverride;
...
public Action OnObjectDeflected(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		BossCharge[boss][0]+=rageMax[client]*25.0/rageMin[client];  //TODO: Allow this to be customizable
		if(BossCharge[boss][0]>rageMax[client])
			BossCharge[boss][0]=rageMax[client];
	}
	return Plugin_Continue;
...
public Action OnTakeDamage(int client, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
	...
	else
	{
		...
		if(boss != -1)
		{
			if(attacker <= MaxClients)
			{
				...
				switch(index)
				{
					...
					case 21, 208, 40, 215, 659, 741, 798, 807, 887, 896, 905, 914, 963, 972, 1146, 15005, 15017, 15030, 15034, 15049, 15054, 15066, 15067, 15068, 15089, 15090, 15115, 15141, 30474:	//火焰喷射器 燃烧加血
					{
						int health=GetClientHealth(attacker);
						int max=GetEntProp(attacker, Prop_Data, "m_iMaxHealth");
						int newhealth=health+9;
						if(health<max+200)
						{
					...
					case 61, 1006:  //Ambassador, Festive Ambassador 大使手枪
					{
						if(kvWeaponMods == null || GetConVarInt(cvarHardcodeWep)>0)
						{
							if(damagecustom == TF_CUSTOM_HEADSHOT)
							{
								damage = 275.0;  //Final damage 255
								return Plugin_Changed;
							}
						}
					}
					...
					case 307:  //Ullapool Caber 木柄手榴弹
					{
						if(GetEntProp(weapon, Prop_Send, "m_iDetonated") == 0)	// If using ullapool caber, only trigger if bomb hasn't been detonated
                        			{
							if(TimesTen)
							{
								damage = (((Pow(float(BossHealthMax[boss]), 0.74074)-(Cabered[client]/128.0*float(BossHealthMax[boss])))/(3+(GetConVarFloat(cvarTimesTen)*allowedDetonations*3)))*bosses)/1.2;
							}
							else if(GetConVarBool(cvarLowStab))
							{
								damage = (((Pow(float(BossHealthMax[boss]), 0.74074)+(2000.0/float(playing))+206.0-(Cabered[client]/128.0*float(BossHealthMax[boss])))/(3+(allowedDetonations*3)))*bosses)/1.2;
							}
							else
							{
								damage = (((Pow(float(BossHealthMax[boss]), 0.74074)+512.0-(Cabered[client]/128.0*float(BossHealthMax[boss])))/(3+(allowedDetonations*3)))*bosses)/1.2;
							}
							damagetype |= DMG_CRIT;
					...
					case 357:  //Half-Zatoichi 座头市
					{
						...
						{
							newhealth = health + RoundToFloor(max/1.8);
							if(health < max2)
							{
								if(newhealth > max2)
									newhealth = max2;
					...
					case 416:  //Market Gardener (courtesy of Chdata) 市场花园铁铲
					{
		...
		else
		{
			...
			if(IsValidClient(client, false) && TF2_GetPlayerClass(client)==TFClass_Soldier)  //TODO: LOOK AT THIS 士兵携带炮艇与靴子的时候
			{
			...
			if(IsValidClient(client, false) && TF2_GetPlayerClass(client)==TFClass_DemoMan)  //TODO: LOOK AT THIS 爆破手没有主武器的时候（假腿）
			{
				if(damagetype & DMG_FALL)
				{
						int secondary = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);	
						if(secondary<=0 || !IsValidEntity(secondary))
						{
							damage/=10.0;
							return Plugin_Changed;
